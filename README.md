...
- 📦 **Archive Extraction** — ZIP/tar/tar.gz uploads are automatically extracted and served as individual files. Supports 200 GB-class archives (tested with a 250 GiB PLATEAU CityGML zip), checkpoint-based resume, per-entry retries, and root folder auto-stripping. Deflate zip entries are transmuxed to gzip objects without recompression, so extraction is network-bound rather than CPU-bound. See [ADR-010](./docs/adr/010-large-archive-extraction-hardening.md) and [ADR-011](./docs/adr/011-recompression-free-extraction.md).
