...
import { getAssetMetadata, updateAsset, setAssetAccess, enrichAssetWithVersion } from "../usecase";
  ...
  app.patch("/:id",
    describeRoute({
      ...
      description:
        ...
        "and the access mode (ADR-013 B7). `{\"access\":\"password\",\"password\":\"…\"}` protects " +
        "a site (archive) asset in a project; `{\"access\":\"public\"}` unprotects it. Setting or " +
        "changing the password invalidates every outstanding auth cookie.",
      responses: {
        200: { description: "Updated asset", content: { "application/json": { schema: resolver(assetResponseSchema) } } },
        400: { description: "Protection is not available for this asset", content: { "application/json": { schema: resolver(errorResponseSchema) } } },
    ...
    async (c) => {
      ...
      // Protection first: it is the only field that can fail for a reason other
      // than "no such asset", and applying the harmless fields before refusing
      // would leave a half-done PATCH behind.
